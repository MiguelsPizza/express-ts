...
export type SplitPath<T extends string> = T extends `/${infer Rest}` ? SplitPath<Rest> : T extends `${infer Segment}/${infer Rest}` ? [Segment, ...SplitPath<Rest>] : T extends '' ? [] : [T];
...
export type BuildRoute<Method extends string, Path extends string, RouteInfo> = BuildNestedRouteType<SplitPath<Path>, Record<Method, RouteInfo>>;
...
export type BuildNestedRoutes<R extends Partial<Record<HttpMethod, Record<string, { query: any; body: any; response: any }>>>> = UnionToIntersection<
  {
    [M in keyof R]: {
