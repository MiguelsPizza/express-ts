...
export type JSONSerializedDoc<T> = T extends (infer U)[]
  ...
  : {
    ...
    K extends MongooseInternalFields ? never : K]:
    T[K] extends Date ? string :
    T[K] extends { _id: any; toString(): string } ? string : // handles ObjectId
    T[K] extends (infer U)[] ? JSONSerializedDoc<U>[] :
    T[K] extends object ? JSONSerializedDoc<T[K]> :
