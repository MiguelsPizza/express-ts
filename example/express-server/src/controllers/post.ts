...
const postRouter = new TypedRouter()
  ...
  .get<"/posts", PostQueyType, void, Post[]>(
    "/posts",
    zValidator('query', postQuery),
    async (req, res) => {
      const { sort, limit } = req.query;

      const results = await Database.instance.select()
        .from(posts)
  ...
  .post<"/posts", void, NewPost, Post>("/posts", zValidator('body', insertPostSchema), async (req, res) => {
    const newPost = await Database.instance.insert(posts)
      .values(req.body)
      .returning();
  ...
  .put<"/posts/:postId", void, NewPost, Post>("/posts/:postId", async (req, res) => {
    ...
    const updatedPost = await Database.instance.update(posts)
      .set({
        ...validation.data,
      })
  ...
  .delete("/posts/:postId", async (req, res) => {
    const postId = parseInt(req.params.postId);
    const deletedPost = await Database.instance.delete(posts)
      .where(eq(posts.id, postId))
      .returning();
